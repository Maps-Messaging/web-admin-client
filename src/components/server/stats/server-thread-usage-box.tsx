...
export function ServerThreadUsageBox ({
                                     data = {},
                                   }: ServerThreadUsageBoxProps): React.JSX.Element {
  return (
    <Card>
      <CardContent>
        <Typography variant="h6">Thread States</Typography>
        {Object.entries(data?.threadState || {}).map(([state, count]) => (
          <Typography key={state} variant="body2">{state}: {count}</Typography>
        ))}
        <Typography variant="body2">Total Threads: {data?.numberOfThreads}</Typography>
      </CardContent>
    </Card>
  );
}
